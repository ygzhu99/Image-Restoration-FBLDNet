...
def main():
    ...
    if 'color' in model_name:
        ...
        nc = 64               # setting for color image
        nb = 15               # setting for color image
    else:
    ...
    # ----------------------------------------
    print(model_path)
    from models.network_fbldnet import FBLDNet as net
    model = net(in_nc=n_channels, out_nc=n_channels, nc=nc, nb=nb, act_mode='NL')
    # print(model)
    ...
    for idx, img in enumerate(L_paths):
        ...
        '''
            加噪声
        '''
        image_sigma = img_L + sigma
        # util.imshow_color(image_sigma)
        img_A = util.tensor2uint(image_sigma)
        util.imsave(img_A,os.path.join(E_path, img_name+'noise'+ext))
        # ------------------------------------
        # (2) img_E
        # ------------------------------------
        ...
        if need_H:
            ...
            img_H = img_H.squeeze()     #？？？？为什么要去掉一个维度
